    ...
    while (1) {
        ...
        if (LPC_TIM1->TC > 60000000) // if one minute lasts, new values are calculated
        {
            ...
            if (counter2 > 10) {
                ...
                if (counter2 ==
                    1) // new average values are written to flash memory only once in every 10 minutes - for the firtst time
                {
        ...
        if (state == 2) // if state == 2 prints data
        {
            ...
            if (average_minute_temp / (float) counter1 < 100 || average_minute_hum / (float) counter1 < 100) {
                ...
                sprintf(xc, "%4.2fC %4.2f %%", average_minute_temp / (float) counter1,
                        average_minute_hum / (float) counter1);
                printString(xc, 200, 15, LCDBlue);
            }

            if (counter2 > 0 &&
                (average_ten_temp / (float) counter2 < 100 || average_ten_hum / (float) counter2 < 100)) {
                printString("Aktualna srednia 10 minutowa:", 170, 15, LCDBlue);
                sprintf(xc2, "%4.2fC %4.2f %%", average_ten_temp / (float) counter2,
                        average_ten_hum / (float) counter2);
